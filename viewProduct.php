...
<head>
   ...
   <link rel="stylesheet" href="style.css">
    <title>view Product | Masterfood</title>
...
<div class="col-3">
    <div class="row">
        <div class="col-6 d-grid pt-2">
            <button class="btn btn-danger" onclick="sort(0);">Sort</button>
        </div>
        <div class="col-6 d-grid pt-2">
...
WHERE `items`.`name`='pots' AND `product`.status_id='1' ");
$product_num_nopg = $product_rs_nopg-> num_rows;
$product_data_nopg =$product_rs_nopg-> fetch_assoc();
$result_per_page = 4;
...
INNER JOIN type_has_items ON `product`.`type_has_items_id` = `type_has_items`.`id`
INNER JOIN items ON `type_has_items`.`items_id` = `items`.`id` 
WHERE `items`.`name`='pots' AND `product`.status_id='1' LIMIT ".$result_per_page." OFFSET ".$page_result." ");
