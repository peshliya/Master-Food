...
<div class="col-3">
    <div class="row">
        <div class="col-6 d-grid pt-2">
            <button class="btn btn-danger" onclick="sortf(0);">Sort</button>
        </div>
        <div class="col-6 d-grid pt-2">
...
INNER JOIN type_has_items ON `product`.`type_has_items_id` = `type_has_items`.`id`
INNER JOIN items ON `type_has_items`.`items_id` = `items`.`id` 
WHERE `items`.`name`='fertilizers' LIMIT ".$result_per_page." OFFSET ".$page_result." ");
